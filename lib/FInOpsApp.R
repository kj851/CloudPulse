...
k8s_selected_cluster            <- NULL
get_mock_k8s_clusters           <- NULL
get_mock_k8s_nodes              <- NULL
.safe_k8s_error                 <- NULL
get_mock_k8s_pods               <- NULL
get_mock_k8s_deployments        <- NULL
...
app_css <- tags$link(rel = "stylesheet", type = "text/css", href = "styles.css")
  ...
  div(
    ...
    div(
      ...
      div(class = "text-center mb-4",
        tags$img(
          src = "logo.png", height = "60px",
          onerror = "this.style.display='none'",
          style = "margin-bottom: 0.5rem;"
        ),

        h1(
          "CloudPulse", class = "fw-bold text-white mb-1",
          style = "font-size: 2.2em;"
        ),

        p(
          "Multi-Cloud FinOps & Infrastructure Platform",
          class = "mb-0",
      ...
      div(class = "card shadow-lg p-4",
        ...
        p(
          "Connect one or more providers simultaneously.",
          ...
          nav_panel("AWS",
            div(class = "pt-3",
              ...
              conditionalPanel("input.connect_aws == true",
                div(class = "mb-2", textInput(
                  "aws_access_key", "Access Key ID",
                  placeholder = "AKIA...", width = "100%"
                )),
                div(class = "mb-2", passwordInput(
                ...
                div(class = "mb-2",
                  checkboxInput(
                    "aws_k8s_enabled", "Include EKS clusters", value = FALSE
                  )
                )
              )
            )
          ...
          nav_panel("Azure",
            div(class = "pt-3",
              ...
              conditionalPanel(
                ...
                div(class = "mb-2",
                  checkboxInput(
                    "azure_k8s_enabled", "Include AKS clusters", value = FALSE
                  )
                )
              )
            )
          ...
          nav_panel("GCP",
            div(class = "pt-3",
              ...
              conditionalPanel("input.connect_gcp == true",
                div(class = "mb-2",
                  textInput("gcp_project_id", "Project ID",
...
multicloud_ui <- function() {
  div(class = "p-4",
    ...
    div(class = "mb-4",
      ...
      p("Unified view of cost, 
      usage, and health across all connected providers.",
        class = "text-muted")
    ),
    ...
    layout_columns(col_widths = c(6, 6),
      card(full_screen = TRUE,
        card_header(tags$span(
          "⚡ CPU Utilization by Provider", class = "fw-bold"
        )),
        card_body(plotlyOutput(
          "multicloud_cpu_plot", height = "260px"
        ) |> withSpinner())
      ),
      card(full_screen = TRUE,
...
developer_ui <- function() {
  div(
      ...
      card(
        ...
        card_body(
          div(class = "mb-3",
            ...
            textInput(
              "dev_org", NULL,
              placeholder = "e.g. Data Engineering", width = "100%"
            )
          ),
          div(class = "mb-3",
          ...
          div(class = "mb-3",
            ...
            numericInput(
              "dev_ttl", NULL, value = 8,
              min = 1, max = 168, width = "100%"
            ),
            tags$small(
          ...
          div(class = "d-grid",
            actionButton(
              "dev_request_btn", "Submit Request",
    ...
    layout_columns(
      ...
      card(
        card_header(tags$span(
          "Automated Instance Shutdown", class = "fw-bold"
        )),
        card_body(
          layout_columns(
            col_widths = c(4, 4, 4),
            ...
            div(
              ...
              div(
                ...
                checkboxInput("sched_offhours",
                  "Enable off-hours shutdown (8PM – 7AM)", value = FALSE
                )
              ),
              conditionalPanel("input.sched_offhours == true",
                div(
                  class = "mb-2",
                  tags$label("Business hours timezone",
            ...
            div(
              ...
              p(
                "Set a specific shutdown time for selected instances.",
                class = "text-muted small"
              ),
              div(
                class = "mb-2",
                tags$label("Shutdown date & time",
                  class = "form-label fw-bold small"
                ),
              ...
              div(
                class = "mb-2",
                tags$label("Target instances (comma-separated)",
                  class = "form-label fw-bold small"
                ),
    ...
    sidebar = sidebar(
      ...
      conditionalPanel("input.current_page_input === 'dashboard'",
        ...
        conditionalPanel(
          ...
          div(class = "mb-4",
            h6(
              "Time Period", class = "fw-bold text-uppercase small text-muted"
            ),
          ...
          div(class = "mb-4",
            ...
            conditionalPanel("input.enable_forecast == true",
              numericInput(
                ...
                value = 7, min = 1, max = 90, width = "100%"
              )
            )
          )
        ),
        div(
          ...
          actionButton(
            "query", "Query Data", class = "btn btn-primary fw-bold",
...
# Analytics content for the main dashboard page,
# including KPIs, data table, and visualizations.
...
server <- function(input, output, session) {
  ...
  instance_requests <- reactiveVal(data.frame(
    ID = character(), Org = character(),
    Provider = character(), Type = character(),
    Region = character(), TTL_hrs = numeric(),
    Status = character(), Requested = character(),
  ...
  output$multicloud_cost_plot <- renderPlotly({
    ...
    if (is.null(df) || nrow(df) == 0)
      return(plot_ly() |>
          add_annotations(text = "No cost data", showarrow = FALSE)
      )
    ...
    for (prov in providers) {
      tryCatch({
        df <- if (use_mock) get_mock_usage(prov, "") else
          inst <- switch(prov,
            AWS   = tryCatch(
              aws_rds_instances_metadata()$identifier[[1]],
              error = function(e) ""
            ),
            Azure = tryCatch(
              azure_db_instances_metadata()$name[[1]],
              error = function(e) ""
            ),
            GCP   = tryCatch(
              gcp_db_instances_metadata()$name[[1]],
              error = function(e) ""
            )
          )
        ...
        if (nrow(df) > 0)
          p_obj <- p_obj |> add_trace(
            x = df$timestamp, y = df$cpu_avg, type = "scatter",
            mode = "lines", name = prov,
            line = list(color = colors[[prov]], width = 2)
          )
      }, error = function(e) NULL)
    }
  ...
  output$multicloud_k8s_summary <- renderUI({
    ...
    if (length(clusters) == 0)
      return(div(
        class = "text-muted text-center py-3", "No clusters connected."
      ))
    tagList(lapply(clusters, function(cl) {
      color <- switch(
        cl$status, Healthy = "#d1fae5",
        Warning = "#fef3c7", Error = "#fee2e2", "#f1f5f9"
      )
      text  <- switch(
        cl$status, Healthy = "#065f46",
        Warning = "#92400e", Error = "#991b1b", "#64748b"
      )
      div(
        ...
        div(
          tags$strong(
            cl$name, style = paste0("font-size:0.85em; color:", text)
          ),
          tags$br(),
          tags$small(paste0(
  ...
  output$multicloud_resources_table <- renderDT({
    ...
    # Normalize each provider to a common 6-column schema before rbind.
    # Each provider returns different column names so rbind would fail.
    normalize_meta <- function(df, provider) {
      if (is.null(df) || nrow(df) == 0) return(NULL)
      data.frame(
        Provider = provider,
        Name     = as.character(
          df[[c(AWS = "identifier", Azure = "name", GCP = "name")[[provider]]]]
        ),
        Status   = as.character(
          df[[c(AWS = "status", Azure = "state", GCP = "state")[[provider]]]]
        ),
        Engine   = as.character(
          if (provider == "AWS")
            df[["engine"]]
          else if (provider == "Azure")
            paste0("PostgreSQL ", df[["version"]])
          else
            df[["database_version"]]
        ),
        Size     = as.character(
          if (provider == "AWS")
            paste0(df[["storage_gb"]], " GB")
          else if (provider == "Azure")
            df[["sku_name"]]
          else
            df[["tier"]]
        ...
        Endpoint = as.character(
          if (provider == "AWS")
            ...
            df[["fully_qualified_domain_name"]]
          else
            df[["connection_name"]]
        ),
  ...
  output$k8s_kpi_cards <- renderUI({
    ...
    pods    <- k8s_pods_data()
    deploys <- k8s_deployments_data() # nolint
    clusters <- k8s_clusters()
  ...
  output$k8s_metrics_plot <- renderPlotly({
    ...
    if (is.null(df) || nrow(df) == 0)
      return(plot_ly() |>
          add_annotations(text = "No metrics", showarrow = FALSE)
      )
  ...
  output$cloud_table <- renderDT({
    ...
    if ("Error" %in% colnames(data))
      return(datatable(data, options = list(dom = "t")))
    datatable(
      data, options = list(autoWidth = FALSE,
  ...
  output$cloud_plot <- renderPlotly({
    ...
    if (input$query_type == "Usage" && nrow(data) > 0 &&
      ...
      p |> layout(hovermode = "x unified",
                 ...
                 nrow(data) > 0 && "cost" %in% colnames(data)) {
      ...
      plot_ly() |> add_annotations(
        text = "No visualization available for this query.",
  ...
  observeEvent(input$dev_request_btn, {
    ...
    new_row <- data.frame(
      ID = new_id, Org = org, Provider = input$dev_provider,
      Type = input$dev_instance_type, Region = input$dev_region,
      TTL_hrs = ttl, Status = "Pending",
      ...
      div(class = "card instance-card mb-2 p-3",
        div(class = "d-flex justify-content-between align-items-start",
          div(tags$strong(row$ID), tags$br(),
            tags$small(paste(
              row$Org, "·", row$Provider, "·", row$Type
            ), class = "text-muted")
          ),

          div(tags$span(
            row$Status, class = paste("status-badge", sc)
          ), tags$br(),
        ...
        if (nzchar(row$Notes))
          tags$small(row$Notes, class = "text-muted d-block mt-1"),

        div(class = "mt-2",
          actionButton(paste0("stop_", i),
            "Stop",
            class = "btn btn-sm btn-outline-danger me-1"
          ),

          actionButton(paste0("extend_", i),
  ...
  observeEvent(input$sched_weekend, {
    sched_log(c(
      ...
      paste0(
        ...
        "] Weekend shutdown ",
        if (input$sched_weekend) "ENABLED" else "DISABLED"
      )
    ))
  })

  observeEvent(input$sched_offhours, {
    tz <- if (isTRUE(input$sched_offhours) &&
                !is.null(input$sched_tz)) input$sched_tz else ""
    sched_log(c(
      sched_log(), paste0("[",
        format(Sys.time(), "%H:%M:%S"),
        "] Off-hours shutdown ",
        if (input$sched_offhours) "ENABLED" else "DISABLED",
  ...
  output$sched_log_ui <- renderUI({
    ...
    tagList(lapply(rev(logs), function(entry) {
      tags$div(
        entry,
        style = "font-family:monospace; padding:2px 0; border-bottom:1px solid rgba(0,0,0,0.05);"
      )
    }))
  })
}
